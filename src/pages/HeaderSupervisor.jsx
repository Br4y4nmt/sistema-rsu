...
const HeaderSupervisor = ({ onToggleSidebar, mostrarSidebar }) => {
  ...
  return (
    <header className="flex justify-between items-center bg-white dark:bg-gray-900 shadow px-4 py-4 w-full">
      {/* Botón hamburguesa solo cuando sidebar está oculto */}
      {!mostrarSidebar ? (
        <button
      ...
      <div className="relative" ref={menuRef}>
        {foto ? (
          <button
            className="w-11 h-11 rounded-full overflow-hidden border-2 border-teal-500 bg-white focus:outline-none"
        ...
        {openMenu && (
          <div className="absolute right-0 mt-2 w-36 bg-white rounded-lg shadow-lg py-1 z-50 border border-gray-100">
            <button
              className="block w-full text-left px-3 py-1.5 hover:bg-[#011B4B] hover:text-white hover:rounded-md text-gray-800 font-semibold bg-white transition-colors focus:outline-none text-sm"
              onClick={() => {
                localStorage.clear();
                setOpenMenu(false);
                navigate('/login');
